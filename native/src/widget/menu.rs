...
pub struct MenuTree<'a, Message, Renderer>{
    ...
    children: Vec<MenuTree<'a, Message, Renderer>>
}
...
struct Menu<'a, 'b, Message, Renderer>{
    ...
    /// expand the children bounds for inside check
    bounds_expand: u16,
        ...
        state.indices.iter()
            ...
            let draw_menu = |r: &mut Renderer|{
                ...
                if i.is_some(){
                    ...
                    let path_color = Color::from([0.0, 0.0, 0.0, 0.3]);
                ...
                let menu_color = Color::from([0.8;3]);
